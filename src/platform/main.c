...
void android_main(struct android_app* app) {
    ...
    Div domino = make_div(
        make_squircle(make_vw(22), make_vw(44), 6),
        STYLE_INIT(.color=0xFFD0E8ED, .left=make_vw(20), .top=make_vh(20), .transform=transform)
    );
    div_add_child(&root, &domino);

    Div bar = make_div(
        make_squircle(make_vw(20), make_vw(.5), 8),
        STYLE_INIT(.color=0xFF000A00, .left=make_vw(1), .top=make_vw(21.75))
    );
    Div cc = make_div(
        make_circle(make_vw(.5)),
        STYLE_INIT(.color=0xFF00D7FF, .left=make_vw(10.5), .top=make_vw(21.5))
    );
    Div c1 = make_div(
        make_circle(make_vw(2)),
